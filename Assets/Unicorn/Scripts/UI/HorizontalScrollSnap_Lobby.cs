...
namespace UnityEngine.UI.Extensions
{
    ...
    public class HorizontalScrollSnap_Lobby : ScrollSnapBase, IEndDragHandler
    {
        void Start()
        {
            _isVertical = false;
        ...
        public void OnEndDrag(PointerEventData eventData)
        {
            ...
            if (_scroll_rect.horizontal)
            {
                ...
                float distance = 4 * (_startPosition.x - _screensContainer.localPosition.x);
                if (UseFastSwipe && (Mathf.Abs(distance) > _childSize || IsMove(SwipeVelocityThreshold)))
                {
